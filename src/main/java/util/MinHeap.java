...
public class MinHeap {
  ...
  /**
   * data数组保存最小堆所有的数据，数据被组织成树形
   * 最大堆满足原则：对于父结点，其左右结点均大于等于父节点
  ...
  public void minHeapify(int rootIndex) {
    ...
    // 从左右两个堆中找出最小结点
    if (leftIndex != -1 && data[leftIndex] < data[smallestIndex])
      smallestIndex = leftIndex;
    if (rightIndex != -1 && data[rightIndex] < data[smallestIndex])
      smallestIndex = rightIndex;

    // 如果都比当前结点大，则当前结点肯定遵循最大堆原则
    // 否则交换最大结点和当前结点的值，之后最大结点可能不再符合最大堆原则，对其进行最大堆化
    if (smallestIndex == rootIndex)
      return;
    else {
      int temp = data[smallestIndex];
      data[smallestIndex] = data[rootIndex];
      data[rootIndex]= temp;
